...
def __start_flet_server(
    ...
    #!!!bundle_path for sandbox 
    bundle_path = os.environ.get("BUNDLE_PATH")
    if bundle_path:
        bundle_resources = bundle_path+os.sep+"Contents"+os.sep+"Resources"
        fletd_in_bundle = os.environ.get("FLETD_BUNDLE_PATH")
        logger.info(f"\n\n\nFLETD PATH is {fletd_in_bundle}")
        assert fletd_in_bundle,"FLETD_BUNDLE_PATH path not in enviroment"
        fletd_path = bundle_resources+fletd_in_bundle+os.sep+fletd_exe
    else:
    ...
    logger.info(f"FLETD ARGS:{args}")
    fletd_proc=subprocess.Popen(
        args,
        env=fletd_env,
    ...
    os.environ["FLETD_PROC_PID"]=str(fletd_proc.pid)
...
def __locate_and_unpack_flet_view(page_url, assets_dir, hidden):
    ...
    if is_windows():
        ...
        if bundle_path:
            bundle_resources = bundle_path+os.sep+"Contents"+os.sep+"Resources"
            fletapp_in_bundle = os.environ.get("FLETAPP_BUNDLE_PATH")
            assert fletapp_in_bundle,"FLETAPP_BUNDLE_PATH path not in enviroment"
            temp_flet_dir = Path(bundle_resources+fletapp_in_bundle)
        else:
...
# def write_flet_page(page_url):
#     flet_page_url_file = tempfile.gettempdir()+os.sep+"FLET_PAGE_URL"
#     try:
#         assert os.path.exists(flet_page_url_file),"Flet Page url file not initialized"
#     except AssertionError as e:
#         logger.error(e)
#     with open(flet_page_url_file,'w') as f:
#         f.write(page_url)
